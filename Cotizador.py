...
if cotizar:
    ...
    for f in filas:
        filas_html += (
            ...
            f"<td>{f['f_ded']:.4f}</td>"
            f"<td class='col-prima'>{fmt_m(f['prima'])}</td>"
            "</tr>"
        )
    ...
    tabla_html = (
        ...
        "<th>Deducible</th><th>Valor Deducible</th><th>f<sub>DED</sub></th><th class='col-prima'>Prima</th>"
        "</tr></thead><tbody>"
        + filas_html +
        "<tr class='total-row'>"
        ...
        "<td colspan='2'></td>"
        "<td>Total Prima</td>"
        f"<td class='col-prima'>{fmt_m(total_prima)}</td>"
        "</tr></tbody></table></div>"
    )
